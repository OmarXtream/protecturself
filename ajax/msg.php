...
ini_set('session.cookie_domain', '.protecturself.org');
...
if($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  if(!isset($_SESSION['_token']) OR !isset($_POST['token']) OR $_POST['token'] != $_SESSION['_token']){
	...
	if(empty($_POST['name']) OR empty($_POST['email']) OR empty($_POST['msg']) ){
		...
		$stmtz=$conn->prepare("INSERT INTO contact (fName,pnum,eMail,msg,date,IP) VALUES (:fName, :pnumber, :eMail, :msg,".time().", :ip)");
    $stmtz->bindValue(":fName", $_POST['name']);
    $stmtz->bindValue(":pnumber", 0);
...
$ip = ''.$info['status'].' » '.$info['message'].' » '.$info['query'];
  ...
  ";
   ...
   returnJSON(array('tp' => 'success', 't' => 'تم', 'm' => 'تم إرسال رسالتك بنجاح، سيتم التواصل معك في القريب العاجل.. ','b' => true));
